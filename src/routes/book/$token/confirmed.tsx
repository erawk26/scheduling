...
export const Route = createFileRoute('/book/$token/confirmed')({
  loader: async ({ params }) => {
    ...
    try {
      ...
      // File not found — proceed without slot detail
    }

    return { status: 'valid' as const, payload: result.payload, selectedSlot }
  },
  component: ConfirmedPageRoute,
})
...
function ConfirmedPageRoute() {
  ...
  const { payload, selectedSlot } = data
