  ...
  return (
    <div data-testid="pending-message" className="flex items-end gap-2 flex-row-reverse">
      ...
      <div className="flex flex-col gap-1 max-w-[75%] items-end">
        <div className="px-4 py-2.5 rounded-2xl rounded-br-sm bg-secondary text-muted-foreground text-sm leading-relaxed whitespace-pre-wrap opacity-70">
          {content}
        </div>
        <div className="flex items-center gap-1 text-xs text-muted-foreground">
          {isStale ? (
            <>
              <AlertTriangle className="w-3 h-3 text-warning-muted-foreground" />
              <span className="text-warning-muted-foreground">Message stale — queued over 72 hours</span>
            </>
          ) : (
